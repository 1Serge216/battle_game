Ниже представлен отформатированный и исправленный код. 

В процессе форматирования были устранены синтаксические ошибки, возникшие при копировании:

В функции generateMap восстановлена пропущенная закрывающая скобка в строке вычисления координаты моста.
В обработчике сообщения endTurn удалена опечатка (room.t !==urn) и добавлены недостающие точки с запятой после вызова calcTerritoryState.
Исправлен оператор сложения строк и чисел (приведение к строке через шаблонные литералы).
Код приведен к единому стилю отступов (2 пробела) для лучшей читаемости серверной логики на Node.js.
...
function getFFABase(role, size) {
  ...
  switch (role) {
    case 'player1':
      return { y0: 0, y1: c - 1, x0: 0, x1: c - 1 };
    case 'player2':
      return { y0: 0, y1: c - 1, x0: size - c, x1: size - 1 };
    case 'player3':
      return { y0: size - c, y1: size - 1, x0: 0, x1: c - 1 };
    case 'player4':
      return { y0: size - c, y1: size - 1, x0: size - c, x1: size - 1 };
  }
  return null;
}
...
function castLine(room, unit, starts, range, opts) {
  ...
  for (const s of starts) {
    for (let k = 0; k < range; k++) {
      ...
      if (blockOwn && room.units.some(o => o.x === nx && o.y === ny && o.hp > 0 && o.owner === unit.owner && o.id !== unit.id)) break;
    }
  }
  return out;
}
...
function refreshWaitingSlot(room) {
  ...
  const canAccept = !room.saved && room.roles.some(r => !room.players[r] && !room.passwords[r]);
...
function createRoom(mode) {
  ...
  const room = {
    ...
    map: generateMap(spec.size, spec.riverWidth, spec.bridges, spec.riverDir, {
      noRiver: !!spec.noRiver,
      extraTerrain: !!spec.extraTerrain
    }),
    units: [],
    phase: 'setup',
    turn: firstTurn,
...
function publicState(room, role) {
  ...
  for (let y = 0; y < size; y++) {
    ...
    for (let x = 0; x < size; x++) {
      ...
      row.push(vis ? { terrain: room.map[y][x].terrain, bridge: room.map[y][x].bridge } : { terrain: 'unknown', bridge: false });
    }
    mapOut.push(row);
  }
    ...
    try {
      ...
      if (msg.type === 'joinQueue') {
        ...
        if (waitingId) {
          ...
          const freeRole = r && !r.saved ? r.roles.find(ro => !r.players[ro] && !r.passwords[ro]) : null;
...
function applyAction(room, role, u, a) {
  try {
    if (a.action === 'move') {
      ...
      if (dx !== 0 && dy !== 0 && Math.abs(dx)) return;
      ...
      if (hitExact && target) {
        ...
        if (target.hp <= 0) {
          ...
          killedTypes[target.id] = target.type;
        }
      }
