...
const MODES = {
    '10v20': {
        size: 15, riverWidth: 1, bridges: 4,
...
function generateMap(size, riverWidth, bridgesCount) {
    ...
    while (placed < bridgesCount && attempts < 500) {
        ...
        if (bestY < 0) continue;
        m[bestY][x].bridge = true;
        if (x + 1 < size && m[bestY][x + 1].terrain === 'river') m[bestY][x + 1].bridge = true;
        usedCols.push(x);
        placed++;
    }
    // −2 холма, +2 леса относительно старого баланса
    const hills = Math.max(6, Math.floor(size * size * 0.035));
    const forests = Math.max(20, Math.floor(size * size * 0.115));
    const place = (t, n) => {
        let c = 0, att = 0;
        while (c < n && att < n * 30) {
            att++;
...
function autoPlaceUnits(room, role) {
    ...
    const riverRow = Math.floor(size / 2);
    const taken = new Set();
    room.units.forEach(u => taken.add(u.y + ',' + u.x));
    const positions = [];
    const rows = [];
    if (isTop) {
        for (let y = 1; y <= riverRow - 2; y++) rows.push(y);
    } else {
        for (let y = size - 2; y >= riverRow + 2; y--) rows.push(y);
    }
    for (const y of rows) {
...
function publicState(room, role) {
    ...
    const units = room.units
        ...
        .map(u => ({ id: u.id, owner: u.owner, x: u.x, y: u.y, dir: u.dir, hp: u.hp, acted: u.acted, type: u.type, reloading: u.reloading }));

    let territory = null;
    if (room.mode === 'capture') {
        const t = calcTerritoryState(room);
        const opp = getOpponent(room, role);
        territory = { me: t[role].percent, enemy: t[opp].percent, myCells: t[role].cells, enemyCells: t[opp].cells };
    }
    ...
    return {
        ...
        turnNumber: room.turnNumber, territory, setupZone
    };
}
...
wss.on('connection', (ws) => {
    ws.on('message', (raw) => {
        ...
        if (msg.type === 'setup') {
            ...
            units.forEach(u => {
                room.units.push({ id: room.nextUnitId++, owner: 'defender', x: u.x, y: u.y, dir: 4, hp: 2, acted: false, type: u.type, reloading: false });
            });
            room.phase = 'battle';
            broadcast(room);
            return;
        ...
        if (msg.type === 'endTurn') {
            ...
            room.units.forEach(x => {
                x.acted = false;
                if (x.owner === getOpponent(room, role)) x.reloading = false;
            });
            room.drawProposed = null;

            if (room.mode === '15v15' && room.spec.drawLimit) {
                const my = room.movesCount[role] || 0;
                const opp = room.movesCount[getOpponent(room, role)] || 0;
    ...
    ws.on('close', () => {
        ...
        if (room) {
            for (const r of Object.keys(room.players)) {
                if (room.players[r] && room.players[r] !== ws) sendTo(room.players[r], { type: 'opponentLeft' });
            }
            rooms.delete(room.id);
        }
    });
...
function applyAction(room, role, u, a) {
    if (a.action === 'move') {
        ...
        u.acted = true;
        room.lastEvent = { kind: 'shot', by: role, from: { x: u.x, y: u.y }, to: { x: t.x, y: t.y }, hit, killed, at: Date.now() };
        ...
        u.acted = true;
        room.lastEvent = { kind: 'shot', by: role, from: { x: u.x, y: u.y }, to: { x, y }, hit, killed, blind: true, at: Date.now() };
        ...
        u.reloading = true;
        room.lastEvent = {
            kind: 'shot', by: role, from: { x: u.x, y: u.y }, to: { x: fx, y: fy },
