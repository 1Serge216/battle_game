...
function autoPlaceUnits(room, role) {
    ...
    cannonPositions.forEach(p => room.units.push({
        id: room.nextUnitId++, owner: role, x: p.x, y: p.y, dir,
...
// Если комната перестала быть пригодной для ожидания новых игроков — снять её со слота ожидания.
...
// Завершение партии: удаляем ВСЕ пароли, снимаем saved.
        ...
        try {
            ...
            if (msg.type === 'joinQueue') {
                ...
                if (waitingId) {
                    ...
                    // Свободен только слот БЕЗ игрока И БЕЗ пароля — слоты с паролем "зарезервированы" под реконнект.
                    ...
                    if (r && !r.saved && freeRole) {
                        ...
                        if (full) {
                            refreshWaitingSlot(r); // закроется, потому что freeRole исчерпаны
                            checkEnd(r);
                            for (const ro of r.roles) {
                                sendTo(r.players[ro], { type: 'matched', role: ro, roomId: r.id, mode });
                            }
                            broadcast(r);
                        } else {
                        ...
                        // нет пригодного слота — освобождаем слот ожидания и создаём новую комнату
            ...
            if (msg.type === 'surrender') {
                ...
                // Удаляем ТОЛЬКО пароль сдавшегося
                ...
                // saved: партия сохранена, если у всех ЖИВЫХ ролей есть пароль
            ...
            if (msg.type === 'leave') {
                ...
                if (room) {
                    ...
                    // пароль НЕ удаляем — игрок может вернуться
                    refreshWaitingSlot(room);
                }
                ws.roomId = null; ws.role = null;
                return;
            ...
            if (msg.type === 'acceptDraw') {
                if (room.phase !== 'battle') return;
                if (!room.drawProposed) return;
                if (room.drawProposed.accepts.includes(role)) return;
                room.drawProposed.accepts.push(role);
                const alive = room.roles.filter(r => room.units.some(u => u.owner === r && u.hp > 0));
                if (alive.every(r => room.drawProposed.accepts.includes(r))) {
            ...
            for (let dy = -1; dy <= 1; dy++) {
                for (let dx = -1; dx <= 1; dx++) {
                    ...
                    if (Math.random() < chance) {
                        ...
                        if (v.hp <= 0 && !killedIds.includes(v.id)) {
                            killedIds.push(v.id);
